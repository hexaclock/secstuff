...
/*
 * returns a ptr to a base-256 char* representation of the num
*/
unsigned char *numtobuf(long int num)
{
  unsigned char *ret;
  unsigned int j;
  ret = (unsigned char *)malloc(BLKSIZE*sizeof(unsigned char));
  memset(ret,0,BLKSIZE);
...
void incrstr(char *str, unsigned int len)
{
  static unsigned char *numbuf;
  static char chk = 0;
  unsigned int i;
  unsigned int j;
  ...
  if (chk == 0)
    {
      numbuf = (unsigned char *)malloc(len*sizeof(unsigned char));
      memset(numbuf,0,len);
      chk = 1;
    }
  i = len - 1;
